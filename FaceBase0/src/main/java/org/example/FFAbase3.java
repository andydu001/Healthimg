...
public class FFAbase3{
    ...
    FFAbase3() {
        ...
        // AwsCredentials identity = (AwsCredentials) AwsCredentialsIdentity.create("","");
        ...
        completableFuture.defaultExecutor().execute(new Runnable() {
            @Override
            public void run() {
                try {
                    System.out.println(completableFuture.copy().get());
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
